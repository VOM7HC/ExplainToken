...
def run(args):
    ...
    explainer = SFATokenSHAP(backend, budget=args.budget, seed=args.seed)
    
    samples = load_data(args.dataset, n=args.n_samples, seed=args.seed)
    results = []
    
    for i, sample in enumerate(samples):
        print(f"[{i+1}/{len(samples)}] {sample.prompt[:50]}...")
    ...
    if len(samples) > 0:
        ...
        for seed in range(args.seed, args.seed + 3):
            exp = SFATokenSHAP(backend, budget=args.budget, seed=seed).explain(samples[0].prompt)
